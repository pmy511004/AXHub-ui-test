...
> **2026-05-15 변경**
> - 홈 페이지 primary color: `#5B3D7A` (보라) → `#2D64FA` (파랑)
> - `--color-gray-50` (사이드바·카드 배경): `#f9f9f9` → `#f6f7f9`

#### 2.1.0 디자인 시스템 토큰 (light/dark 자동 전환)

다음 토큰은 [globals.css](src/app/globals.css) `:root` 와 `.dark-mode` 셀렉터에 정의되어 있으며, `@theme inline`을 통해 Tailwind 유틸리티(`bg-primary`, `text-fg-default`, `border-border-default`, `bg-bg-content` 등)로 자동 노출됩니다.

**Primary**

| 토큰 | Light | Dark | 용도 |
|------|-------|------|------|
| `--p-primary` | `#2D64FA` | `#4B7BFF` | 주요 액션, 활성 상태 |
| `--p-primary-hover` | `#5985FB` | `#2D64FA` | 호버 (라이트는 밝게, 다크는 어둡게) |
| `--p-primary-soft` | `#EAF0FE` | `rgba(75,123,255,0.16)` | Soft 버튼·선택 상태 카드 fill |
| `--p-primary-soft-hover` | `#D6E2FD` | `rgba(75,123,255,0.24)` | Soft 호버 |
| `--p-primary-fg` | `#FFFFFF` | `#FFFFFF` | primary 위 텍스트/아이콘 |

**Foreground (text)**

| 토큰 | Light | Dark | 용도 |
|------|-------|------|------|
| `--p-fg-default` | `#16171B` | `#FAFAFA` | 본문·타이틀 |
| `--p-fg-secondary` | `#3C3F47` | `#D1D3D8` | 보조 헤딩·아웃라인 버튼 |
| `--p-fg-muted` | `#6D717E` | `#9CA0AB` | 헬퍼·캡션·카테고리 |
| `--p-fg-subtle` | `#9CA0AB` | `#6D717E` | placeholder·비활성 탭 라벨 |
| `--p-fg-disabled` | `#D1D3D8` | `#4F535D` | disabled 라벨 |
| `--p-fg-on-primary` | `#FFFFFF` | `#FFFFFF` | primary 위 텍스트/아이콘 |

**Border**

| 토큰 | Light | Dark | 용도 |
|------|-------|------|------|
| `--p-border-default` | `#E4E5E8` | `rgba(255,255,255,0.10)` | 기본 1px 헤어라인 |
| `--p-border-strong` | `#D1D3D8` | `rgba(255,255,255,0.16)` | 좀 더 또렷한 보더 |
| `--p-border-divider` | `rgba(82,82,91,0.08)` | `rgba(255,255,255,0.08)` | 탭 underline·리스트 separator |

**Background (Surface)**

| 토큰 | Light | Dark | 용도 |
|------|-------|------|------|
| `--p-bg-surface` | `#F6F7F9` | `#0E1014` | 페이지 캔버스 |
| `--p-bg-content` | `#FFFFFF` | `#16181D` | 카드·모달 주표면 |
| `--p-bg-muted` | `#EFF1F4` | `#1D2026` | 서치·코드블록 |
| `--p-bg-emphasis` | `#E4E5E8` | `#25282F` | 호버·secondary 버튼 |
| `--p-bg-inverse` | `#16171B` | `#FAFAFA` | 툴팁·반전 칩 |

> Tailwind 유틸리티로 쓰려면 `bg-primary`, `text-fg-muted`, `border-border-strong`, `bg-bg-content` 등을 사용합니다. 다크 전환은 `.dark-mode` 클래스가 부모에 적용되면 변수 값만 swap되어 자동으로 반영돼요.

| 토큰 | 값 | 용도 |
|------|-----|------|
| `--color-primary-50` | `#f4ecfa` | primary 배경 연한 |
| `--color-primary-500` | `#6d319d` | 팀 아이콘 고정색 |
| `--page-primary` | 페이지별 동적 | 각 페이지의 활성 메뉴 인디케이터 색상 (홈은 `#2D64FA`) |
| `--color-gray-50` | `#f6f7f9` | 사이드바·카드 배경 |
...
|--------|--------------------------|------------------------|--------------------------------|------|
| 홈 (`/`) | `#2D64FA` | `#6E4A94` | `#130321` → `#0C0A12` | 활성 메뉴 텍스트, 인디케이터 바, 모달 CTA |
...
> 홈 페이지의 primary color는 2026-05-15부터 보라(#5B3D7A) → 파랑(#2D64FA)으로 변경되었습니다. 다크 모드 텍스트의 primary 파랑 `#2D64FA`는 한 단계 밝은 `#9866CC`로 자동 전환됩니다(§11 참고). 그 외 페이지 primary는 동일한 hex로 유지하되, 배경 표면이 어두워지면 시각적으로 더 강조됩니다.
...
| 모달 액션 | `h-9 px-5 text-sm` | `취소` = `bg-[#f6f6f6] text-[#18181b]` / `만들기` = `bg-[#2D64FA] text-white` |
| 토글 칩 (예: 최초접속/사용 중) | `h-9 px-4 text-sm` | 활성 `bg-[#2D64FA] text-white`, 비활성 `text-[#71717a] hover:text-[#18181b]` |
...
```tsx
<button className="relative overflow-hidden bg-[#2D64FA] text-white ...">
  버튼 텍스트
...
- 인풋: `min-h-12 rounded-full border border-[#e4e4e7] px-4 text-base placeholder:text-[#a1a1aa] focus:border-[#2D64FA]`
...
- 액션 (우측 하단): 취소(`bg-[#f6f6f6]`) + 만들기(`bg-[#2D64FA]`, 비활성 시 `bg-white/70` 오버레이)
...
| Raised / 카드 (`bg-[#f6f6f6]`, `bg-[#f4f4f5]`, `bg-[#f6f7f9]`, `bg-gray-50/100`) | 회색 50~100 | `#1c1b21` | 자동 변환 |
| 모달 / 팝오버 (`rounded-xl/-2xl` + `bg-white`) | `#ffffff` | `#16151a` + `1px rgba(255,255,255,0.1)` 윤곽 | 자동 변환 |
| Hover 표면 (`hover:bg-[#f4f4f5]`, `hover:bg-[#f6f7f9]`, `hover:bg-[#ececec]`, `hover:bg-gray-50`) | 회색 50~100 | `#2a2932` | 자동 변환 |
...
| Primary 파랑 텍스트 | `#2D64FA` | `#9866CC` |
| 카테고리 탭 비활성 텍스트/숫자 | `rgba(24,24,27,0.9)` / `rgba(24,24,27,0.48)` | `#fafafa` / `#d4d4d8` |

> 보더·배경의 `--page-primary`(`#2D64FA → #6E4A94`)와 별개로, **텍스트의 primary 파랑색만 한 단계 밝은 `#9866CC`로** 분리해 어두운 표면에서 대비를 확보합니다. `.menu-active-accent`도 동일 규칙.
...
- **배너 ellipse 그라데이션**: 라이트 center `#2D64FA` / outer `#F6F6F6` → 다크 center `#9866CC` / outer `#1C1B21`
